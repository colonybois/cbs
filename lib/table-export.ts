...
const pdfCell = (value: ExportRow[number]) =>
  cell(value)
    .replaceAll("₹", "Rs. ")
    .replace(/[^\x20-\x7E]/g, "");
...
export function exportTable(
  ...
  if (format === "csv") {
    const csv = [headers, ...rows]
      .map((row) => row.map((value) => `"${cell(value).replaceAll('"', '""')}"`).join(","))
      .join("\n");
  ...
  if (format === "excel") {
    ...
    sheet["!cols"] = headers.map((header, index) => ({
      wch: Math.min(
        40,
        Math.max(header.length + 2, ...rows.map((row) => cell(row[index]).length + 2)),
      ),
    }));
  ...
  autoTable(pdf, {
    head: [headers.map(pdfCell)],
    body: rows.map((row) => row.map(pdfCell)),
    startY: 64,
