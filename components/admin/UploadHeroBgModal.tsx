...
export default function UploadHeroBgModal({
  open,
  onClose,
}: {
  open: boolean;
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (role !== "admin" && role !== "super_admin") {
      setError("Only verified admins can change the hero background.");
      return;
    }
    if (!file) {
    ...
    try {
      ...
      await setDoc(
        ...
        { heroBgUrl, updatedAt: serverTimestamp() },
        { merge: true },
      );
      setFile(null);
      onClose();
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : "Could not update the hero background.");
    } finally {
      setUploading(false);
    }
  };
  return (
    <Modal open={open} onClose={onClose} title="Update hero background">
      <form onSubmit={submit} className="space-y-4">
        <fieldset disabled={uploading} className="space-y-4 disabled:opacity-60">
          ...
          {previewUrl && (
            <div className="relative aspect-[16/8] overflow-hidden rounded-xl border border-orange-200">
              <img
                src={previewUrl}
                alt="New hero background preview"
                className="h-full w-full object-cover"
              />
              <span className="absolute right-3 top-3 rounded-full bg-slate-950/70 px-3 py-1 text-xs font-bold text-white">
        ...
        <Button disabled={uploading} type="submit" className="w-full">
          {uploading ? "Uploading background..." : "Save hero background"}
        </Button>
      </form>
    </Modal>
  );
