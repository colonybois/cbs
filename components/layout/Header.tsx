...
const superAdminLinks = [["👑 Super Admin Dashboard", "/admin/super-dashboard"], ...adminLinks] as const;
...
export default function Header() {
  ...
  const dashboardPath = role === "admin" || role === "super_admin" ? "/admin/dashboard" : "/member/dashboard";
  ...
  const authActions = loading ? <span className="h-9 w-28 animate-pulse rounded-lg bg-orange-100"/> : signedIn ? <>{!inPortal && <><Link href={dashboardPath} className="rounded-lg bg-orange-500 px-4 py-2 text-sm font-bold text-white hover:bg-orange-600">🛠️ Admin Dashboard</Link>{role === "super_admin" && <Link href="/admin/super-dashboard" className="rounded-lg border border-amber-300 bg-amber-50 px-3 py-2 text-sm font-bold text-amber-800 hover:bg-amber-100">👑 Super Admin</Link>}</>}<button onClick={() => void handleSignOut()} className="rounded-lg border border-orange-300 px-3 py-1.5 text-sm font-bold text-orange-700 hover:bg-orange-50">Sign Out</button></> : <>{!registrationConfig.hideRegistrationUI && <Link href="/register" className="text-sm font-bold text-orange-700 hover:text-orange-600">Register</Link>}<Link href="/login" className="rounded-lg bg-orange-500 px-4 py-2 text-sm font-bold text-white hover:bg-orange-600">Sign In</Link></>;
  return <header className="sticky top-0 z-50 border-b-2 border-orange-500 bg-white/95 shadow-sm backdrop-blur"><div className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-3 px-4">{brand}<nav className="hidden items-center gap-5 md:flex">{inPortal ? portalNav : publicSections.map(([label, id]) => <button key={id} onClick={() => scrollToSection(id)} className={`text-sm font-medium hover:text-orange-600 ${id === "donate" ? "font-bold text-orange-600" : "text-slate-700"}`}>{label}</button>)}{instagramIcon}{authActions}</nav><div className="flex items-center gap-1 md:hidden">{instagramIcon}<button onClick={() => setIsMobileMenuOpen(value => !value)} aria-expanded={isMobileMenuOpen} aria-label="Toggle navigation menu" className="p-2 text-slate-800">{isMobileMenuOpen ? "✕" : "☰"}</button></div></div>{isMobileMenuOpen && <div className="flex flex-col space-y-1 border-b-2 border-orange-500 bg-white px-4 py-4 text-left shadow-xl md:hidden">{inPortal ? portalNav : publicSections.map(([label, id]) => <button key={id} onClick={() => scrollToSection(id)} className={`rounded-lg px-3 py-2 text-left font-medium ${id === "donate" ? "font-bold text-orange-600" : "text-slate-800 hover:bg-orange-50"}`}>{label}</button>)}{loading ? <span className="mx-3 mt-2 h-10 animate-pulse rounded-lg bg-orange-100"/> : signedIn ? <div className="flex gap-2 px-3 pt-2">{!inPortal && <Link href={dashboardPath} onClick={closeMenu} className="flex-1 rounded-lg bg-orange-500 py-2 text-center font-bold text-white">Dashboard</Link>}<button onClick={() => void handleSignOut()} className="rounded-lg border border-orange-300 px-3 py-2 text-sm font-bold text-orange-700">Sign Out</button></div> : <div className="flex gap-2 px-3 pt-2">{!registrationConfig.hideRegistrationUI && <Link href="/register" onClick={closeMenu} className="flex-1 rounded-lg border border-orange-300 py-2 text-center font-bold text-orange-700">Register</Link>}<Link href="/login" onClick={closeMenu} className="flex-1 rounded-lg bg-orange-500 py-2 text-center font-bold text-white">Sign In</Link></div>}</div>}</header>;
}
