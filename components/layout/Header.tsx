...
export default function Header() {
  ...
  const brand = (
    ...
    >
      <span className="grid h-10 w-10 place-items-center overflow-hidden rounded-full border border-orange-200 bg-orange-50">
        {!imgError ? (
          <img
  ...
  return (
    <header className="sticky top-0 z-50 border-b-2 border-orange-500 bg-white/95 shadow-sm backdrop-blur">
      ...
      {isMobileMenuOpen && (
        <div className="flex flex-col space-y-1 border-b-2 border-orange-500 bg-white px-4 py-4 text-left shadow-xl md:hidden">
          {inPortal
            ? portalNav
            : publicSections.map(([label, id]) => (
                <button
                  ...
                  className={`rounded-lg px-3 py-2 text-left font-medium ${id === "donate" ? "font-bold text-orange-600" : "text-slate-800 hover:bg-orange-50"}`}
                >
                  {label}
                </button>
              ))}
          {loading ? (
            <span className="mx-3 mt-2 h-10 animate-pulse rounded-lg bg-orange-100" />
          ) : signedIn ? (
            <div className="flex gap-2 px-3 pt-2">
              {!inPortal && (
                <Link
                  ...
                  className="flex-1 rounded-lg bg-orange-500 py-2 text-center font-bold text-white"
                >
                  Dashboard
                </Link>
              )}
            ...
            <div className="flex gap-2 px-3 pt-2">
              {!registrationConfig.hideRegistrationUI && (
                <Link
                  ...
                  className="flex-1 rounded-lg border border-orange-300 py-2 text-center font-bold text-orange-700"
                >
                  Register
                </Link>
              )}
