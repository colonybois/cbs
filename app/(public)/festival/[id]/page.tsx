...
export default function FestivalDayDetail() {
  ...
  if (!day) return (
    <div className="mx-auto max-w-4xl py-20 text-center">
      ...
      <Link href="/#gallery" className="mt-6 inline-block rounded-xl bg-orange-500 px-6 py-3 font-bold text-white hover:bg-orange-600">Back to Festival Journey</Link>
    </div>
  );
  ...
  return (
    <div className="mx-auto max-w-4xl py-6 space-y-8">
      ...
      {/* Back */}
      <Link href="/#festival" className="inline-flex items-center gap-2 text-sm font-bold text-orange-600 hover:text-orange-700">
        ← Festival Journey
      </Link>
      ...
      <div className="relative overflow-hidden rounded-3xl border border-orange-200 shadow-xl shadow-orange-100/50">
        ...
        <div className="absolute bottom-0 left-0 p-6">
          <span className="rounded-xl bg-orange-500 px-3 py-1 text-sm font-black text-white shadow">{day.dayLabel}</span>
          {day.featured && <span className="ml-2 rounded-xl bg-amber-400 px-3 py-1 text-sm font-black text-white shadow">★ Featured</span>}
      ...
      {(day.shortDesc || day.detailedDesc) && (
        <div className="rounded-2xl border border-orange-100 bg-white p-6 shadow-sm space-y-3">
          ...
          {day.detailedDesc && <p className="text-slate-600 leading-relaxed whitespace-pre-line">{day.detailedDesc}</p>}
        </div>
      )}
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            {day.galleryUrls.map((url, i) => (
              <button key={i} onClick={() => setLightbox(url)} className="group overflow-hidden rounded-2xl border border-orange-100 bg-orange-50">
                <img
                  src={url}
                  alt={`Gallery ${i + 1}`}
                  loading="lazy"
      ...
      <div className="grid gap-4 sm:grid-cols-2 border-t border-orange-100 pt-6">
        {prev ? (
          <Link href={`/festival/${prev.id}`} className="group flex items-center gap-3 rounded-2xl border border-orange-200 bg-white p-4 hover:border-orange-400 hover:shadow-md transition">
            <span className="text-orange-400 text-xl">←</span>
            <div>
              <p className="text-xs font-bold uppercase text-slate-400">Previous</p>
              <p className="font-bold text-slate-800 group-hover:text-orange-600">{prev.dayLabel} · {prev.heading}</p>
            </div>
          </Link>
        ) : <div />}
        {next ? (
          <Link href={`/festival/${next.id}`} className="group flex items-center justify-end gap-3 rounded-2xl border border-orange-200 bg-white p-4 hover:border-orange-400 hover:shadow-md transition">
            <div className="text-right">
              <p className="text-xs font-bold uppercase text-slate-400">Next</p>
              <p className="font-bold text-slate-800 group-hover:text-orange-600">{next.dayLabel} · {next.heading}</p>
            </div>
            <span className="text-orange-400 text-xl">→</span>
          </Link>
        ) : <div />}
      ...
      {/* Gallery lightbox */}
      {lightbox && (
        <div className="fixed inset-0 z-[60] grid place-items-center bg-slate-950/90 p-4" onClick={() => setLightbox(null)}>
          ...
          <img src={lightbox} alt="Gallery" className="max-h-[85vh] max-w-full rounded-2xl object-contain" onClick={e => e.stopPropagation()} />
        </div>
      )}
    </div>
  );
