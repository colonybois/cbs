...
export default function Login() {
  ...
  useEffect(() => {
    ...
    if (params.get("mode") === "register" && !registrationConfig.hideRegistrationUI) setMode("register");
    if (params.get("notice") === "registration-hidden") setError("Registration is currently unavailable. Please contact the administration.");
  ...
  useEffect(() => {
    if (!registrationConfig.allowedRoles.includes(requestedRole)) setRequestedRole(registrationConfig.allowedRoles[0] || "member");
  }, [registrationConfig.allowedRoles, requestedRole]);

  const routeFor = (userRole: UserRole) => router.push(userRole === "admin" || userRole === "super_admin" ? "/admin/dashboard" : "/member/dashboard");
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!registrationConfig.allowedRoles.includes(requestedRole)) throw new Error("ROLE_REGISTRATION_DISABLED");
      ...
      const msg =
        code === "REGISTRATION_CLOSED" ? "Registrations are currently closed by the administration." :
        code === "ROLE_REGISTRATION_DISABLED" ? "Registration for that role is currently unavailable." :
  ...
  const regClosed = !registrationConfig.isRegistrationOpen || registrationConfig.allowedRoles.length === 0;
  ...
  return <div className="mx-auto mt-8 max-w-md rounded-3xl border border-orange-200 bg-white p-7 shadow-xl shadow-orange-100/50">
    <div className="flex rounded-xl bg-orange-50 p-1">
      ...
      {!registrationHidden && <button onClick={() => changeMode("register")} disabled={regClosed} className={`flex-1 rounded-lg py-2 text-sm font-bold disabled:cursor-not-allowed ${mode === "register" ? "bg-white text-orange-700 shadow-sm" : regClosed ? "text-slate-400" : "text-slate-600"}`}>
    ...
    <p className="mt-2 text-sm text-slate-600">{mode === "signin" ? "Sign in with your organizer or volunteer account." : "New accounts require admin approval."}</p>
    {!(regClosed && mode === "register") && (
      <form onSubmit={submit} className="mt-6 space-y-3">
        ...
        {mode === "register" && <label className="block text-sm font-semibold text-slate-700">Requested role<select value={requestedRole} onChange={event => setRequestedRole(event.target.value as RegistrationRole)} className="mt-1 w-full rounded-xl border border-orange-200 bg-white p-3">{registrationConfig.allowedRoles.map(role => <option key={role} value={role}>{role === "admin" ? "Admin" : "Volunteer member"}</option>)}</select></label>}
