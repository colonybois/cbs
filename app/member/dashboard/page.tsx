...
function ReceiptModal({ d, name, onClose }: { d: Donation; name: string; onClose: () => void }) {
  const receiptText = [
    ...
    d.paymentMode === "upi"
      ? `Payment: ${d.paymentVerificationStatus === "verified" ? "Verified" : "Verification pending"}`
      : null,
    ...
    const unsub = onSnapshot(
      ...
      (snap) => {
        const next = snap.docs
          ...
          .sort((a, b) => {
            const at = dateOf(a.createdAt);
            const bt = dateOf(b.createdAt);
      ...
      () => {
        setLoadError("Unable to load your collection records. Please try again.");
        setLoading(false);
      },
    );
    return unsub;
    ...
    <div className="space-y-8">
        ...
        <div className="fixed inset-0 z-[70] grid place-items-center bg-slate-900/70 p-4">
          <div className="w-full max-w-sm rounded-3xl border-2 border-emerald-400 bg-white p-8 text-center shadow-2xl">
            ...
            <div className="mt-4 rounded-xl bg-emerald-50 p-4 text-left text-sm space-y-1">
              <p>
                <span className="font-semibold">Donor:</span> {justApproved.residentName}
              </p>
              <p>
                <span className="font-semibold">Amount:</span>{" "}
                <strong className="text-emerald-700">
                  ₹{Number(justApproved.amount || 0).toLocaleString()}
                </strong>
              </p>
              <p>
                <span className="font-semibold">Reference:</span>{" "}
                <span className="font-mono text-xs">
                  {justApproved.receiptNo ?? justApproved.id.slice(0, 10)}
                </span>
              </p>
            </div>
      ...
      <div>
        ...
        <p className="mt-1 text-slate-500">
          Log door-to-door collections and track their approval status.
        </p>
      </div>
      ...
      {!loading && (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {[
            ...
            {
              label: "Approved UPI",
              value: `₹${approvedUpi.toLocaleString()}`,
              color: "text-emerald-600",
            },
          ].map((c) => (
            <div
          ...
          {!loading && !loadError && donations.length === 0 && (
            <div className="rounded-2xl border border-dashed border-orange-300 bg-orange-50 p-8 text-center">
              ...
              <p className="mt-1 text-sm text-slate-500">
                Your logged collections will appear here.
              </p>
            </div>
          )}
          {!loading && !loadError && donations.length > 0 && (
                ...
                >
                  <div className="min-w-0">
                    ...
                    <p className="text-xs font-semibold text-slate-500">
                      {(d.paymentMode ?? "cash").toUpperCase()} · Ref:{" "}
                      {d.paymentTransactionId ?? d.receiptNo ?? d.id.slice(0, 10)}
                    </p>
                    {d.paymentMode === "upi" && (
                      <p
      ...
      {/* Receipt modal */}
      {receipt && (
        <ReceiptModal d={receipt} name={name || "Volunteer"} onClose={() => setReceipt(null)} />
      )}
    </div>
  );
}
