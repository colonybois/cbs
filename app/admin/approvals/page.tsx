...
export default function Approvals() {
  ...
  useEffect(() => {
    const unsub = onSnapshot(
      ...
      snap => { setRegistrations(snap.docs.map(d => ({ id: d.id, ...d.data() } as PendingUser))); setLoading(false); },
      () => { setError("Unable to load registration approvals."); setLoading(false); }
    );
    return unsub;
  }, []);
  ...
  return (
    <div>
      <div>
        <p className="text-sm font-bold uppercase tracking-widest text-orange-600">🛡️ Admin review</p>
        <h1 className="mt-2 text-3xl font-black text-slate-900">Registration Approvals</h1>
        <p className="mt-2 text-slate-600">Approve or decline new volunteer and admin account requests.</p>
      </div>

      {error && <p role="alert" className="mt-5 rounded-lg bg-rose-50 px-3 py-2 text-sm font-medium text-rose-700">{error}</p>}

      <div className="mt-7 space-y-3">
        {loading ? (
          <Card className="p-6 text-center text-slate-500">Loading pending registrations…</Card>
        ) : registrations.length === 0 ? (
          <Card className="p-8 text-center text-slate-600">No registrations awaiting approval.</Card>
        ) : registrations.map(user => (
          <Card className="flex flex-wrap items-center gap-4 p-5" key={user.id}>
            <span className="grid h-11 w-11 place-items-center rounded-xl bg-orange-50 text-lg">🛡️</span>
            <div className="flex-1">
              <b className="text-slate-900">{user.name || "Unnamed candidate"}</b>
              <p className="text-sm text-slate-600">
                {user.email || "No email supplied"} · Requested {user.role === "admin" ? "Admin" : "Member"}
              </p>
              <p className="mt-1 text-xs text-slate-500">Registered: {formatDate(user.createdAt)}</p>
            </div>
