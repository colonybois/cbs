...
export default function PaymentLedgerPage() {
  ...
  const collectors = useMemo(() => {
    ...
    donations.forEach((d) => {
      if (d.collectorId && d.collectorName) seen.set(d.collectorId, d.collectorName);
    });
    ...
    return donations.filter((d) => {
      ...
      if (ts) {
        ...
        if (dateFilter === "yesterday") {
          ...
          if (ts < startOf(y) || ts >= startOf(now)) return false;
        }
  ...
  const summary = [
    ...
    [
      `₹${sum(approvedD.filter((d) => d.paymentMode === "cash")).toLocaleString()}`,
      "Approved Cash",
    ],
  ...
  const approve = async (d: Donation) => {
    if (!uid || !adminName || d.collectorId === uid) {
      setError("You cannot approve your own collection.");
      return;
    }
    ...
    try {
      await runTransaction(db, async (tx) => {
        ...
        if (typeof collectorId !== "string" || !collectorId)
          throw new Error("This collection has no valid collector.");
        ...
        if (!Number.isFinite(amount) || amount <= 0)
          throw new Error("This collection has an invalid amount.");
        const paymentMode = snap.data()?.paymentMode;
        if (paymentMode !== "cash" && paymentMode !== "upi")
          throw new Error("This collection has an invalid payment method.");
  ...
  const reject = async () => {
    ...
    try {
      ...
      await recordAudit({
        ...
        newValue: {
          volunteer: rejectTarget.collectorName,
          donor: rejectTarget.residentName,
  ...
  const deleteSelected = async () => {
    ...
    if (targets.length > 100) {
      setError("Delete up to 100 ledger entries at a time.");
      return;
    }
    if (
      !window.confirm(
        `Permanently delete ${targets.length} selected ledger entr${targets.length === 1 ? "y" : "ies"}? Approved totals will be reversed. This cannot be undone.`,
      )
    )
      return;
    setDeleting(true);
    ...
    try {
      ...
      await recordAudit({
        actorId: uid,
        actorName: adminName,
        action: "Deleted payment ledger entries",
        module: "Payment Ledger",
        newValue: { count: targets.length, donationIds: targets.map((item) => item.id) },
        approvalStatus: "approved",
      });
      setSelectedIds([]);
    } catch (err) {
      setError(
        err instanceof Error ? err.message : "Could not delete the selected ledger entries.",
      );
    } finally {
      setDeleting(false);
    }
  ...
  return (
    <div className="space-y-7">
      <WelcomeBanner
        ...
        text="Review and approve volunteer door-to-door collections. Only approved collections count toward the official total."
      />
        ...
        <div className="flex flex-wrap gap-3">
          ...
          {canDelete && (
            <button
              ...
              className="rounded-xl border border-rose-300 bg-rose-50 px-4 py-2.5 text-sm font-bold text-rose-700 hover:bg-rose-100 disabled:opacity-50"
            >
              {deleting
                ? "Deleting…"
          ...
          <div className="flex flex-wrap gap-3">
            <label className="text-sm text-slate-600">
              ...
              <input
                ...
                className="ml-1 rounded-lg border border-slate-300 px-2 py-2 text-sm"
              />
            </label>
            <label className="text-sm text-slate-600">
              To{" "}
                  ...
                  <tr key={d.id} className="hover:bg-white transition">
                    {canDelete && (
                      <td className="px-4 py-3">
                        <input
                          aria-label={`Select ${d.residentName || "ledger entry"}`}
                          type="checkbox"
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`rounded-full px-2 py-0.5 text-xs font-bold ${STATUS_STYLE[d.status ?? "pending_approval"] ?? "bg-slate-100 text-slate-500"}`}
                      >
                      ...
                      {d.status === "approved" && d.approvedByName && (
                        <p className="mt-0.5 text-xs text-slate-400">by {d.approvedByName}</p>
                      )}
                      {d.status === "rejected" && d.rejectionReason && (
                        <p className="mt-0.5 text-xs text-rose-400">{d.rejectionReason}</p>
                      )}
                    </td>
                    {/* Actions */}
                    <td className="px-4 py-3">
        ...
        >
          ...
          >
            ...
            <div className="space-y-2 text-sm text-slate-700">
              {(
                [
                  ...
                  [
                    "Status",
                    receiptTarget.status === "approved"
                      ? "✅ Verified"
                      : receiptTarget.status === "rejected"
                        ? "❌ Rejected"
                        : "⏳ Pending",
                  ],
                  receiptTarget.approvedByName
                    ? ["Approved by", receiptTarget.approvedByName]
                    : null,
            ...
            <p className="mt-4 text-center text-xs text-slate-400">
              Colony Bois · Ganesh Chaturthi Chanda
            </p>
          </div>
        </div>
      )}
