...
type Transaction = {
  id: string; residentName?: string; donorEmail?: string | null;
  phone?: string; amount?: number; transactionId?: string | null;
  message?: string | null; paymentMethod?: string; screenshotUrl?: string | null;
  showPublicName?: boolean; emailStatus?: string;
  status?: "pending" | "approved" | "rejected";
  createdAt?: TimestampLike; verifiedAt?: TimestampLike; verifiedBy?: string; verifiedByName?: string;
};
type Filter = "all" | "approved" | "pending" | "rejected";

const dateOf = (v: TimestampLike | undefined) => v && typeof v !== "string" ? v.toDate?.() : v ? new Date(v) : null;
const displayDate = (v: TimestampLike | undefined) => dateOf(v)?.toLocaleString() || "Not available";
const statusLabel = (s: Transaction["status"]) => s === "approved" ? "Successful" : s === "rejected" ? "Failed" : "Pending";
...
export default function ChandaTransactionsPage() {
  ...
  const review = async (transaction: Transaction, status: "approved" | "rejected") => {
    ...
    try {
      ...
      batch.update(doc(db, "online_donations", transaction.id), {
        status, verifiedBy: uid, verifiedByName: name, verifiedAt: new Date().toISOString(),
      });
  ...
  const triggerEmail = async (
    ...
    await updateDoc(doc(db, "online_donations", t.id), { emailStatus: result.ok ? "sent" : "failed" });
    if (result.ok) {
      await recordAudit({ actorId, actorName, action: "Donation email sent", module: "Chanda Transactions", targetId: t.id, newValue: { recipient: t.donorEmail, type, status: "sent" } });
    }
    setEmailingId("");
  };
  ...
  const syncExistingSupporters = async () => {
    if (!uid) return;
    setSyncingSupporters(true); setError("");
    try {
      const eligible = transactions.filter(t => t.status === "approved" && t.showPublicName === true);
      if (eligible.length > 500) throw new Error("Too many supporters to sync at once.");
      const batch = writeBatch(db);
      eligible.forEach(t => batch.set(doc(db, "public_supporters", t.id), { displayName: t.residentName || "Anonymous supporter", message: t.message || null, approvalStatus: "approved", publicDisplay: true, updatedAt: new Date().toISOString() }));
      await batch.commit();
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Could not sync public supporters."); }
  ...
  return (
    <div className="space-y-7">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        {summary.map(([v, l]) => <Card key={l} className="bg-white p-5"><p className="text-2xl font-black text-orange-600">{v}</p><p className="mt-2 text-sm text-slate-600">{l}</p></Card>)}
      </div>
      ...
      <Card className="p-5">
        <div className="flex flex-wrap items-center gap-3">
          <select value={filter} onChange={e => setFilter(e.target.value as Filter)} className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm">
            <option value="all">All</option><option value="approved">Successful</option><option value="pending">Pending</option><option value="rejected">Failed</option>
          </select>
            ...
            {filtered.map(item => (
              <tr key={item.id} className="align-top">
                ...
                <td className="px-4 py-4">
                  {item.screenshotUrl
                    ? <button onClick={() => setLightbox(item.screenshotUrl!)} className="group relative overflow-hidden rounded-lg border border-orange-200 bg-orange-50 hover:border-orange-400 transition"><img src={item.screenshotUrl} alt="Proof" className="h-12 w-16 object-cover"/><span className="absolute inset-0 hidden items-center justify-center bg-slate-900/40 text-xs font-bold text-white group-hover:flex">View</span></button>
                    : <span className="rounded-lg bg-slate-100 px-2 py-1 text-xs text-slate-500">No screenshot</span>}
                </td>
                {/* Donor */}
                <td className="px-4 py-4 font-semibold text-slate-900">
                  {item.residentName || "Unknown donor"}
                  <br/><span className="font-normal text-slate-500">{item.phone || "No phone"}</span>
                </td>
                <td className="px-4 py-4 font-bold text-orange-700">₹{Number(item.amount || 0).toLocaleString()}</td>
                ...
                <td className="px-4 py-4 text-slate-600">{item.paymentMethod?.toUpperCase() || "UPI"}</td>
                {/* Status */}
                <td className="px-4 py-4">
                  <span className={`rounded-full px-2 py-1 text-xs font-bold ${item.status === "approved" ? "bg-emerald-50 text-emerald-700" : item.status === "rejected" ? "bg-rose-50 text-rose-700" : "bg-amber-50 text-amber-700"}`}>
                    {statusLabel(item.status)}
                  </span>
                  <p className="mt-1 text-xs text-slate-500">{item.verifiedAt ? `Reviewed ${displayDate(item.verifiedAt)}` : "Awaiting"}</p>
                </td>
                {/* Email status + retry */}
                <td className="px-4 py-4">
                  {item.donorEmail ? (
                    <div className="space-y-1">
                      <span className={`rounded-full px-2 py-0.5 text-xs font-bold ${EMAIL_BADGE[item.emailStatus ?? "not_sent"] ?? EMAIL_BADGE.not_sent}`}>
                        {item.emailStatus === "sent" ? "✓ Sent" : item.emailStatus === "failed" ? "✕ Failed" : item.emailStatus === "queued" ? "… Queued" : "Not sent"}
                      </span>
                      {(item.emailStatus === "failed" || item.emailStatus === "not_sent") && item.status === "approved" && uid && name && (
                        <button disabled={emailingId === item.id} onClick={() => void triggerEmail(item, "receipt", uid, name)}
                ...
                <td className="px-4 py-4">
                  {item.status === "pending"
                    ? <div className="flex gap-2">
                        <button disabled={updatingId === item.id} onClick={() => void review(item, "approved")} className="rounded-lg bg-orange-500 px-3 py-2 text-xs font-bold text-white disabled:opacity-50">Verify</button>
                        <button disabled={updatingId === item.id} onClick={() => void review(item, "rejected")} className="rounded-lg border border-rose-300 px-3 py-2 text-xs font-bold text-rose-600 disabled:opacity-50">Reject</button>
                      </div>
                    : <span className="text-xs text-slate-500">{item.verifiedByName || item.verifiedBy || "Reviewed"}</span>}
                </td>
              </tr>
            ))}
      ...
      {lightbox && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/90 p-4" onClick={() => setLightbox(null)}>
          <button onClick={() => setLightbox(null)} className="absolute right-5 top-5 flex h-9 w-9 items-center justify-center rounded-full bg-white text-slate-900 font-bold hover:bg-orange-50">✕</button>
          <img src={lightbox} alt="Payment screenshot" className="max-h-[85vh] max-w-full rounded-2xl object-contain shadow-2xl" onClick={e => e.stopPropagation()} />
        </div>
      )}
    </div>
  );
