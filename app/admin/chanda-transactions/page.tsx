...
type Transaction = { id: string; residentName?: string; phone?: string; amount?: number; transactionId?: string | null; message?: string | null; paymentMethod?: string; screenshotUrl?: string | null; showPublicName?: boolean; status?: "pending" | "approved" | "rejected"; createdAt?: TimestampLike; verifiedAt?: TimestampLike; verifiedBy?: string };
...
export default function ChandaTransactionsPage() {
  ...
  const review = async (transaction: Transaction, status: "approved" | "rejected") => {
    ...
    try {
      ...
      batch.update(doc(db, "online_donations", transaction.id), { status, verifiedBy: uid, verifiedByName: name, verifiedAt: new Date().toISOString() });
      ...
      if (status === "approved" && transaction.showPublicName === true) {
        ...
        // A rejected or private payment must never have a public projection.
  ...
  const syncExistingSupporters = async () => {
    ...
    try {
      const eligible = transactions.filter(item => item.status === "approved" && item.showPublicName === true);
      if (eligible.length > 500) throw new Error("Too many supporters to sync at once. Please contact an administrator.");
      const batch = writeBatch(db);
      eligible.forEach(item => batch.set(doc(db, "public_supporters", item.id), {
        displayName: item.residentName || "Anonymous supporter",
  ...
  return <div className="space-y-7"><WelcomeBanner title="Chanda Transactions" text="Only verified transactions are counted as collected. A UPI app launch is not payment confirmation." />
    ...
    <Card className="p-5"><div className="flex flex-wrap items-center gap-3"><select value={filter} onChange={event => setFilter(event.target.value as Filter)} className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm"><option value="all">All</option><option value="approved">Successful</option><option value="pending">Pending</option><option value="rejected">Failed</option></select><label className="text-sm text-slate-600">From <input type="date" value={from} onChange={event => setFrom(event.target.value)} className="ml-1 rounded-lg border border-slate-300 px-2 py-2" /></label><label className="text-sm text-slate-600">To <input type="date" value={to} onChange={event => setTo(event.target.value)} className="ml-1 rounded-lg border border-slate-300 px-2 py-2" /></label><button disabled={syncingSupporters} onClick={() => void syncExistingSupporters()} className="rounded-lg border border-emerald-300 bg-emerald-50 px-3 py-2 text-sm font-bold text-emerald-700 disabled:opacity-50">{syncingSupporters ? "Syncing…" : "Sync approved supporters"}</button></div><p className="mt-3 text-xs text-slate-500">One-time backfill for approved donors who opted into public display. Future approvals sync automatically.</p></Card>
