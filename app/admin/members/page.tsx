...
export default function MembersManagementPage() {
  ...
  const [statusFilter, setStatusFilter] = useState<
    "all" | "pending_approval" | "approved" | "rejected" | "flagged"
  >("all");
          ...
          setMembers(
            snap.docs.map((d) => {
              ...
              return {
                ...
                phone: data.phone || "",
                upiTotal: Number(data.upiTotal || 0),
                cashTotal: Number(data.cashTotal || 0),
                pendingHandover: Number(data.pendingHandover || 0),
              };
            }),
          );
          setLoading(false);
  ...
  const memberRows = members.map((member) => {
    const approved = allDonations.filter(
      (d) => d.collectorId === member.id && d.status === "approved",
    );
    const cashTotal = approved
      .filter((d) => d.paymentMode === "cash")
      .reduce((sum, d) => sum + Number(d.amount || 0), 0);
    const upiTotal = approved
  ...
  useEffect(() => {
    ...
    return onSnapshot(
      ...
      (snap) => {
        setDonations(
          snap.docs
            ...
            .sort(
              (a, b) =>
                (dateOf(b.createdAt)?.getTime() ?? 0) - (dateOf(a.createdAt)?.getTime() ?? 0),
            ),
        );
      ...
      () => {
        setDonations([]);
        setHistoryError("Unable to load this member's transaction history.");
        setHistoryLoading(false);
      },
    );
  }, [selected]);
  ...
  const filteredDonations = donations.filter((d) => {
    ...
    if (
      ...
      !d.receiptNo?.toLowerCase().includes(search) &&
    ...
    if (
      ...
      return false;
    if (dateFilter === "7d" && (!date || date < new Date(now.getTime() - 7 * 86400000)))
      return false;
    if (dateFilter === "30d" && (!date || date < new Date(now.getTime() - 30 * 86400000)))
      return false;
    if (
      dateFilter === "custom" &&
    ...
    <div className="mx-auto max-w-7xl space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500">
          Click a collector to view their ledger, transaction history, and manage cash handovers.
        </p>
        <TableExportButtons
          className="mt-3"
                ...
                memberRows.map((m) => (
                  ...
                  >
                    <td className="px-6 py-4 font-bold text-slate-900">
                      {m.name}
                      <span className="block text-xs font-normal text-slate-500">{m.phone}</span>
                    </td>
      ...
      <Modal
        ...
        title={selected ? `${selected.name} — Ledger` : ""}
      >
        {selected && (
          <div className="space-y-5 max-h-[75vh] overflow-y-auto pr-1">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2 text-sm">
                ...
                <div className="rounded-xl bg-emerald-50 p-3">
                  <p className="text-emerald-700 text-xs">Approved UPI</p>
                  <b>
                    ₹{sum(filteredApproved.filter((d) => d.paymentMode === "upi")).toLocaleString()}
                  </b>
                </div>
                {filteredRejected.length > 0 && (
                  <div className="rounded-xl bg-rose-50 p-3">
                    <p className="text-rose-700 text-xs">Rejected</p>
                    <b>₹{sum(filteredRejected).toLocaleString()}</b>
                  </div>
                )}
                <div className="rounded-xl bg-slate-50 p-3">
                  <p className="text-slate-600 text-xs">Total Transactions</p>
                  <b>{filteredDonations.length}</b>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-xl border border-orange-200 bg-orange-50 p-4 text-sm space-y-1">
              ...
              <div className="flex justify-between">
                ...
                <b>₹{sum(approvedCash).toLocaleString()}</b>
              </div>
              <div className="flex justify-between">
            ...
            {outstanding > 0 && (
              ...
              >
                {handingOver
                  ? "Confirming…"
                  : `Confirm Cash Handover ₹${outstanding.toLocaleString()}`}
              </button>
            )}
            ...
            <div>
              ...
              <div className="space-y-2 max-h-64 overflow-y-auto">
                {historyLoading ? (
                  <p className="text-sm text-slate-500 text-center py-4">Loading transactions…</p>
                ) : historyError ? (
                  <p className="text-sm text-rose-600 text-center py-4">{historyError}</p>
                  ...
                  filteredDonations.map((d) => (
                    <div
                      key={d.id}
                      className="flex items-start justify-between gap-2 rounded-xl border border-slate-100 bg-white p-3 text-sm"
                    >
                      <div>
                        ...
                        <p className="text-xs font-mono text-slate-400">
                          Ref: {d.paymentTransactionId ?? d.receiptNo ?? d.id.slice(0, 10)}
                        </p>
