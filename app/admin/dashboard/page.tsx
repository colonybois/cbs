...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const registration = onSnapshot(doc(db, "site_settings", "registration"), snapshot => setRegistrationsOpen(snapshot.data()?.registrationsOpen !== false));
    return () => { users(); online(); field(); registration(); };
  }, []);
  ...
  const toggleRegistrations = async () => {
    ...
    try {
      ...
      await setDoc(doc(db, "site_settings", "registration"), { registrationsOpen: nextValue, updatedAt: new Date().toISOString() }, { merge: true });
      if (uid && name) await recordAudit({ actorId: uid, actorName: name, action: "Changed registration setting", module: "Settings", previousValue: { registrationsOpen }, newValue: { registrationsOpen: nextValue } });
  ...
  const upiTotal = approvedField.filter(item => item.paymentMode === "upi").reduce((sum, item) => sum + Number(item.amount || 0), 0) + onlineUpi;
  ...
  const cards = [[rupees(totalCollected), "Total collected"], [rupees(cashTotal), "Collected via cash"], [rupees(upiTotal), "Collected via UPI"], [rupees(unsettled), "Unsettled field cash"], [String(pending), "Pending payment reviews"], [String(active), "Active collection Bois"]];
  ...
  return <div className="space-y-7">
    ...
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-6">{cards.map(([value, label]) => <Card key={label} className="bg-white p-5"><p className="text-2xl font-black text-orange-600">{value}</p><p className="mt-2 text-sm text-slate-600">{label}</p></Card>)}</div>
    <Card className="flex flex-wrap items-center justify-between gap-4 border-orange-200 bg-orange-50 p-5"><div><p className="text-sm font-bold text-slate-900">Registration access</p><p className="mt-1 text-sm text-slate-600">New volunteer registration is currently <b>{registrationsOpen ? "open" : "closed"}</b>.</p></div><button disabled={savingRegistrationSetting} onClick={() => void toggleRegistrations()} className={`rounded-xl px-4 py-2.5 text-sm font-bold text-white disabled:opacity-60 ${registrationsOpen ? "bg-rose-600 hover:bg-rose-700" : "bg-emerald-600 hover:bg-emerald-700"}`}>{savingRegistrationSetting ? "Saving..." : registrationsOpen ? "Stop registrations" : "Open registrations"}</button></Card>
