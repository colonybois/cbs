...
function Toast({ msg, type }: { msg: string; type: "success" | "error" }) {
  return (
    <div
      className={`fixed bottom-6 right-6 z-[100] max-w-sm rounded-2xl px-5 py-3 text-sm font-bold text-white shadow-xl ${type === "success" ? "bg-emerald-600" : "bg-rose-600"}`}
    >
      {type === "success" ? "✓ " : "✕ "}
      {msg}
    </div>
  ...
  useEffect(
    () =>
      onSnapshot(
        ...
        (snap) => {
          setMembers(snap.docs.map((d) => ({ id: d.id, ...d.data() }) as ComityMember));
          setLoading(false);
        },
        () => setLoading(false),
      ),
  ...
  const submit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (editTarget) {
        ...
        if (editTarget.designation !== form.designation)
          changes["Designation"] = `${editTarget.designation} → ${form.designation}`;
        if (editTarget.publicVisible !== form.publicVisible)
          changes["Visibility"] =
            `${editTarget.publicVisible ? "Public" : "Hidden"} → ${form.publicVisible ? "Public" : "Hidden"}`;
        if (editTarget.featured !== form.featured)
          changes["Featured"] =
            `${editTarget.featured ? "Yes" : "No"} → ${form.featured ? "Yes" : "No"}`;
        if (editTarget.active !== form.active)
          changes["Status"] =
            `${editTarget.active ? "Active" : "Inactive"} → ${form.active ? "Active" : "Inactive"}`;
        await recordAudit({
          actorId: uid,
          actorName: adminName || uid,
          action: "Committee member updated",
          module: "Committee Members",
          targetId: editTarget.id,
          previousValue: { name: editTarget.fullName },
          newValue: { name: form.fullName, changes },
        });
        showToast("Committee member updated.", "success");
      } else {
        const ref = await addDoc(collection(db, "comity_members"), {
          ...payload,
          createdBy: adminName || uid,
          createdAt: serverTimestamp(),
        });
        await recordAudit({
          actorId: uid,
          actorName: adminName || uid,
          action: "Committee member created",
          module: "Committee Members",
          targetId: ref.id,
          newValue: { name: form.fullName, designation: form.designation },
        });
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await recordAudit({
        actorId: uid,
        actorName: adminName || uid,
        action: "Committee member deleted",
        module: "Committee Members",
        targetId: deleteTarget.id,
        previousValue: { name: deleteTarget.fullName, designation: deleteTarget.designation },
      });
  ...
  const moveOrder = async (member: ComityMember, dir: "up" | "down") => {
    ...
    if (uid)
      await recordAudit({
        actorId: uid,
        actorName: adminName || uid,
        action: "Committee member order changed",
        module: "Committee Members",
        targetId: member.id,
        newValue: { name: member.fullName, direction: dir },
      });
  };
  ...
  const filtered = members
    ...
    .filter((m) => {
      ...
      if (filter === "leadership")
        return DESIGNATION_GROUPS["President / Leadership"].includes(m.designation);
      if (filter === "coordinators")
        return DESIGNATION_GROUPS["Coordinators"].includes(m.designation);
      if (filter === "committee")
        return DESIGNATION_GROUPS["Committee Members"].includes(m.designation);
      return true;
    })
  ...
  return (
    <div className="space-y-7">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-slate-500">
            Manage the Colony Bois Ganesh Utsav committee members.
          </p>
        </div>
              ...
              <tbody className="divide-y divide-orange-50">
                {filtered.map((m) => (
                  <tr key={m.id} className="hover:bg-orange-50/40 transition">
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`rounded-full px-2.5 py-1 text-xs font-bold ${m.active ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-500"}`}
                      >
      ...
      <Modal
        ...
        title={editTarget ? "Edit Committee Member" : "Add Committee Member"}
      >
        <form onSubmit={submit} className="max-h-[72vh] overflow-y-auto pr-1 space-y-4">
          <fieldset disabled={saving} className="space-y-4 disabled:opacity-60">
              ...
              {photoPreview ? (
                <div className="relative mt-2 inline-block">
                  ...
                  <button
                    ...
                    onClick={() => {
                      setPhotoFile(null);
                      setPhotoPreview(editTarget?.photoUrl || null);
            ...
            <label className="block text-sm font-semibold text-slate-700">
              ...
              <span className="font-normal text-slate-400">
                (phone/Instagram, shown only if enabled)
              </span>
              <input
                {...f("publicContact")}
          ...
          <div className="flex gap-3 pt-1">
            <Button disabled={saving} type="submit" className="flex-1">
              {saving ? "Saving…" : editTarget ? "Update Member" : "Save Comity Member"}
            </Button>
            <button
              type="button"
        ...
        >
          ...
          >
            ...
            <p className="mt-3 text-sm text-slate-500">
              This will permanently remove them from the public website and admin dashboard.
            </p>
